...
using Puniemu.Src.Server.GameServer.Requests.UpdateCollectMenu.DataClasses;
...
namespace Puniemu.Src.Server.GameServer.Requests.UpdateCollectMenu.Logic
{
    public class UpdateCollectMenuHandler
    {

        public static async Task HandleAsync(HttpContext ctx)
        {

            // not implemented this is initCollectMenu
            ctx.Request.EnableBuffering();
            var readResult = await ctx.Request.BodyReader.ReadAsync();
            var encRequest = Encoding.UTF8.GetString(readResult.Buffer.ToArray());
            ctx.Request.BodyReader.AdvanceTo(readResult.Buffer.End);
            var requestJsonString = NHNCrypt.Logic.NHNCrypt.DecryptRequest(encRequest);
            var deserialized = JsonConvert.DeserializeObject<UpdateCollectMenuRequest>(requestJsonString!);
            ...
            var res = new UpdateCollectMenuResponse(userdata!);
